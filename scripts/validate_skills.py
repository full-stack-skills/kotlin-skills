#!/usr/bin/env python3
"""Validate kotlin-skills structure and optionally execute golden Gradle builds."""
...
def inspect_repository() -> tuple[list[str], list[Path]]:
    ...
    for name in actual:
        ...
        found = sorted((skill_dir / "examples").glob("golden-*/build.gradle.kts"))
        if not found:
            errors.append(f"skills/{name}: missing examples/golden-*/build.gradle.kts")
    ...
    for readme in (ROOT / "README.md", ROOT / "README.zh-CN.md"):
        ...
        if str(len(actual)) not in text or "kotlin-stable" not in text:
            errors.append(f"{readme.name}: skill count or kotlin-stable entry is stale")
    return errors, manifests
...
def check_examples(manifests: list[Path]) -> list[str]:
    ...
    if gradle is None:
        return ["Gradle is required for --check-examples"]
    errors: list[str] = []
    with tempfile.TemporaryDirectory(prefix="kotlin-skills-") as temporary:
        for index, manifest in enumerate(manifests):
            ...
            if completed.returncode:
                errors.append(f"Gradle check failed in {source.relative_to(ROOT)}:\n{completed.stdout}")
            else:
